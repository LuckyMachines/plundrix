export default function IntegrationDebugTrace({ session }) {
  ...
  return (
    <details className="border border-vault-border rounded bg-vault-dark/50 p-3">
      <summary className="cursor-pointer font-mono text-xs uppercase tracking-beacon text-vault-text-dim">
        Integration Trace
      </summary>
      <pre className="mt-3 max-h-72 overflow-auto rounded bg-black/30 p-3 font-mono text-micro leading-relaxed text-vault-text-dim">
        {JSON.stringify(
          {
            mode: session.mode,
