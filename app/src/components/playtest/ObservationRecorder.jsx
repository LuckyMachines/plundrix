...
const INITIAL = { participantCode: 'P-01', observerConfirmed: false, returningPlayer: false, completedFirstAction: false, understoodGoal: false, understoodWhy: false, noticedGadget: false, wantedReplay: false, joyScore: 3, delightMoment: 'none-yet', friction: 'none', secondsToFirstAction: 0 };
    ...
    <section className="rounded border border-oxide-green/35 bg-oxide-green/5 p-4 sm:p-5">
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-[1fr_340px]">
        <div>
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2">{[['completedFirstAction', 'Completed first action unaided'], ['understoodGoal', 'Could explain the win condition'], ['understoodWhy', 'Could explain why the result happened'], ['noticedGadget', 'Noticed the gadget effect'], ['wantedReplay', 'Asked to replay or continue'], ['returningPlayer', 'Returning player']].map(([key, label]) => <label key={key} className="flex min-h-[48px] items-center gap-3 rounded border border-vault-border bg-vault-dark/55 px-3 text-sm text-vault-text"><input type="checkbox" checked={form[key]} onChange={(event) => update(key, event.target.checked)} />{label}</label>)}</div>
        ...
        <aside className="grid grid-cols-2 gap-2"><EvidenceMetric label="Sessions" value={summary.count} /><EvidenceMetric label="Goal clear" value={`${summary.goalRate}%`} /><EvidenceMetric label="Result clear" value={`${summary.whyRate}%`} /><EvidenceMetric label="Gadget seen" value={`${summary.gadgetRate}%`} /><EvidenceMetric label="Would replay" value={`${summary.replayRate}%`} /><EvidenceMetric label="First move" value={`${summary.averageSeconds}s`} /><EvidenceMetric label="Average joy" value={`${summary.averageJoy}/5`} /><EvidenceMetric label="Returning" value={summary.returningCount} /><EvidenceMetric label="Balance samples" value={telemetry.samples} /></aside>
      </div>
    </section>
  );
}
