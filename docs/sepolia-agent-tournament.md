...
- **Tool Hoarder** (`tool-hoarder`) builds three tools before converting them into picks.
- **Leader Hunter** (`leader-hunter`) sabotages decisive breakaways and advances when the race is compressed.
- **Saboteur** (`saboteur`) steals tools and applies stuns, then pivots to picks late to prevent endless disruption loops.
...
During execution the runner atomically refreshes:

- `reports/sepolia-agent-tournament/state.json` - structured schedule, decisions, state transitions, receipts, gas, costs, and winners;
- `reports/sepolia-agent-tournament/report.md` - current standings, action mix, pacing, game ledger, method, and limitations.
